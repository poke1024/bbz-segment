...
from . import GroundTruth, Label
from .utils.transform import Remap

def warp_images(ground_truth, name):
    ...
    def get_corners(x_dir, im_shape):
        ...
        h, w = im_shape[:2]
        pagecoords = numpy.array([[w, h], [0, h], [0, 0], [w, 0]])
    ...
    class Warper:
        def __init__(self, shape):
            rough_dims, self._params = get_default_params(
                get_corners(numpy.array([1, 0]), shape))
        ...
        def __call__(self, kind, im, alpha=0.0, beta=0.0):
            ...
            image_x_coords = cv2.resize(image_x_coords, (self._width, self._height),
                                        interpolation=cv2.INTER_CUBIC)

            image_y_coords = cv2.resize(image_y_coords, (self._width, self._height),
                                        interpolation=cv2.INTER_CUBIC)
                
            remap = Remap(image_x_coords, image_y_coords)
                
            if kind == "labels":
                return remap.labels(im, GroundTruth.label_weights(), border=0)
            elif kind == "image":
                return remap.grayscale(im, border=(255, 255, 255)).astype(im.dtype)
            else:
                raise RuntimeError("unnsupported data type %s" % kind)
